    ...
    public Task BeforeStartAsync(DistributedApplicationModel appModel, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var healthChecksUIResource in healthChecksUIResources)
        {
            foreach (var monitoredProject in healthChecksUIResource.MonitoredProjects)
            {
                ...
                if (!project.Resource.TryGetEndpoint(monitoredProject.EndpointName, out var _))
                {
                ...
                project.WithEnvironment(context =>
                {
                    ...
                    else if (context.ExecutionContext.IsPublishMode)
                    {
                        ...
                        {
                            if (healthChecksEndpoint.TargetPort is int targetPort && targetPort != DefaultAspNetCoreContainerPort)
                            {
                                // The target port is different from the default port so set ASPNETCORE_HTTP_PORTS to listen on both ports
                                context.EnvironmentVariables.Add(ASPNETCORE_HTTP_PORTS, $"{DefaultAspNetCoreContainerPort};{healthChecksEndpoint.Property(EndpointProperty.TargetPort).ValueExpression}");
                            }
                        }
                    }
